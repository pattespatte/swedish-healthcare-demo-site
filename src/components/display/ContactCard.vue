<template>
	<div
		class="bg-white rounded-lg shadow-md overflow-hidden border border-neutral-200"
	>
		<div class="p-6">
			<div class="flex items-start">
				<div v-if="image || $slots.image" class="flex-shrink-0">
					<slot name="image">
						...
						<div
							v-else
							class="h-16 w-16 rounded-full bg-neutral-200 flex items-center justify-center"
						>
							<svg
			...
			<div v-if="email || phone || address" class="mt-6 space-y-3">
				<div v-if="email" class="flex items-start">
					<svg
						class="h-5 w-5 text-neutral-400 mr-3 mt-0.5"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
						aria-hidden="true"
