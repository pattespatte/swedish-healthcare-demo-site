		...
		>
			<div class="space-y-1 text-center">
				...
				<div class="flex text-sm text-gray-600 dark:text-dark-text-secondary">
					<label
						:for="id"
						class="relative cursor-pointer rounded-md bg-white font-medium text-primary-700 focus-within:outline-none hover:text-primary-600 dark:bg-dark-bg-secondary dark:text-primary-400 dark:hover:text-primary-300"
					>
...
<script setup lang="ts">
	...
	const fileTypeInfo = computed(() => {
		...
		return `Stödda format: ${formattedTypes.join(', ')}. Max storlek: ${formatFileSize(props.maxFileSize)}.`
	})
	...
	const handleFileChange = (event: Event) => {
		...
		selectedFiles.forEach(file => {
			...
			if (file.size > props.maxFileSize) {
				errors.push(`Filen "${file.name}" är för stor. Max storlek är ${formatFileSize(props.maxFileSize)}.`)
				return
			}
			...
			if (props.accept) {
				...
				if (!isAccepted) {
					errors.push(`Filen "${file.name}" har inte ett giltigt filformat.`)
					return
				}
			}
