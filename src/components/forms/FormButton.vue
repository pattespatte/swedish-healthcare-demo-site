<template>
	...
	>
		...
		<PhSpinner v-if="loading" size="16" class="-ml-1 mr-2 animate-spin" />

		<!-- Button icon based on type/variant -->
		<component v-if="!loading && showIcon" :is="getButtonIcon()" :size="16" class="mr-2" />
		...
		switch (props.type) {
			...
			default:
				if (props.text) {
					...
					if (text.includes('kontakta') || text.includes('contact')) {
						return 'PhPhone'
					}
					if (text.includes('logga') || text.includes('login')) {
						return 'PhSignIn'
					}
					if (text.includes('spara') || text.includes('save')) {
						return 'PhFloppyDisk'
					}
					if (text.includes('avboka') || text.includes('cancel')) {
						return 'PhX'
					}
					if (text.includes('radera') || text.includes('delete')) {
						return 'PhTrash'
					}
					if (text.includes('ändra') || text.includes('edit')) {
						return 'PhPencil'
					}
					if (text.includes('visa') || text.includes('view')) {
						return 'PhEye'
					}
					if (text.includes('ladda') || text.includes('download')) {
						return 'PhDownload'
					}
					if (text.includes('sök') || text.includes('search')) {
