...
const filteredClinics = computed(() => {
	return specialistClinics.filter((clinic) => {
		...
		const matchesSpecialty =
			...
			clinic.name
				.toLowerCase()
				.includes(selectedSpecialty.value.toLowerCase());
	...
	<div class="container mx-auto px-4 py-4 mb-8 md:mb-16 lg:mb-24">
		...
		>
			<p class="text-gray-600 mb-6">
				Våra specialistmottagningar erbjuder avancerad medicinsk vård
				inom olika områden. Alla våra specialister är erfarna och
				uppdaterade med den senaste kunskapen inom sina fält.
			</p>
					...
					<div class="mt-4 space-y-2">
						<div class="flex justify-between items-center">
							...
							<Badge
								:text="clinic.status"
								:variant="
									clinic.status.includes('Tar emot')
										? 'success'
										: 'secondary'
								"
							/>
		...
		<!-- Specialists view -->
		<div
			v-else-if="activeTab === 'specialists'"
			class="bg-white rounded-lg shadow-md p-6"
		>
			<h2 class="text-xl font-semibold mb-4">Våra specialister</h2>
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				<ContactCard
		...
		<div
			...
			class="bg-white rounded-lg shadow-md p-6"
		>
			<h2 class="text-xl font-semibold mb-4">Remissinformation</h2>
			<div class="space-y-4">
				<ListItem
				...
				<div v-else-if="bookingStep === 4" class="space-y-4">
					...
					<div class="bg-gray-50 p-4 rounded-md">
						<p class="text-sm text-gray-700">
							<strong>Mottagning:</strong> {{ selectedClinic
							}}<br />
							<strong>Anledning:</strong> Remiss för
							specialistvård<br />
							<strong>Personuppgifter:</strong> [Dina
							uppgifter]<br />
							<strong>Symptom:</strong> [Dina symptom]
						</p>
					</div>
				</div>
			</div>
